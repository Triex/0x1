...
import { handleNodeModulesRequest } from './handlers/node-modules-handler';
  ...
  function scanDirectory(
    ...
    try {
      ...
      for (const subdir of subdirs) {
        ...
        // ENHANCED: Handle Next.js route groups: (group) - don't affect URL structure
        if (subdir.startsWith('(') && subdir.endsWith(')')) {
          const groupName = subdir.slice(1, -1);
          logger.debug(`Found route group: ${groupName} at ${subdirPath}`);
          ...
          logger.debug(`Processing regular directory: ${subdir} -> ${subroutePath}`);
...
`;
      ...
      // Handle ALL node_modules with real file serving
      if (reqPath.startsWith("/node_modules/")) {
        const nodeModulesResponse = await handleNodeModulesRequest(reqPath, options.projectPath);
        if (nodeModulesResponse) {
          logRequestStatus(nodeModulesResponse.status, reqPath, `📦 Node modules: ${reqPath.split('/').pop()}`);
